...
def unik_frasa(njajal_p):
    hasil_unik = []
    for x in njajal_p:
        hasil_unik_sem = []
        for y in x:
            hasil_unik_sem.append(list(set(y)))
        hasil_unik.append(hasil_unik_sem)
    return (hasil_unik)
...
def susun_jenis_kata(jumlah_jk, jenis_kata, minim, data):
    ...
    for x, y in jenis_kata.items():
        ...
        if x[:4]=='Konj':
            tempkonj.append(y)
    hasil_subjek, hasil_objek, hasil_predikat = [], [], []
    if len(temps) > minim:
        hasil_subjek = cari_frasa(temps, minim, data)
        if (len(hasil_subjek) != 0):
            hasil_subjek = unik_frasa(hasil_subjek)
    if (len(temps) <= minim) or (len(hasil_subjek)==0):
        hasil_subjek.append(temps)
    if len(tempp) > minim:
        hasil_predikat = cari_frasa(tempp, minim, data)
        if (len(hasil_predikat) != 0):
            hasil_predikat = unik_frasa(hasil_predikat)
    if (len(tempp) <= minim) or (len(hasil_predikat)==0):
        hasil_predikat = []
        hasil_predikat.append(tempp)
    if len(tempo) > minim:
        hasil_objek = cari_frasa(tempo, minim, data)
        if (len(hasil_objek) != 0):
            hasil_objek = unik_frasa(hasil_objek)
    if (len(tempo) <= minim) or (len(hasil_objek)==0):
        hasil_objek.append(tempo)
    if len(tempket) > minim:
        hasil_keterangan = cari_frasa(tempket, minim, data)
        if (len(hasil_keterangan) != 0):
            hasil_keterangan = unik_frasa(hasil_keterangan)
    elif len(tempket) <= minim:
        hasil_keterangan = []
        hasil_keterangan.append(tempket)
    if len(tempkonj) > minim:
        hasil_konjungsi = cari_frasa(tempkonj, minim, data)
        if (len(hasil_konjungsi) != 0):
            hasil_konjungsi = unik_frasa(hasil_konjungsi)
...
def pemisah_kalimat(data, kalimat, count_jenis_kata, subjek_to_objek):
    ...
    for i, x in enumerate(kalimat):
        ...
        if (i==panjang-1) or (x==' '):
            flag = 1
        if ((y in data) and (flag==1)) or ((y in subjek_to_objek) and(flag==1)):
            if (y in data):
                count[data[y]] += 1
                jenis.update({str(data[y])+str(count[data[y]]):y})
            if (y in subjek_to_objek):
                count[subjek_to_objek[y]] += 1
                jenis.update({str(subjek_to_objek[y])+str(count[subjek_to_objek[y]]):y})
            y = ''
            flag = 0
            continue
    return count, jenis

def susun_kalimat(kombinasi_kata, struktur_kalimat, jenis, cocok, jumlah_kata):
    hasil_kalimat_total = []
    for i in kombinasi_kata:
        ...
        for x in struktur_kalimat:
            ...
            if (x=='O') and (len(tempo) != 0):
                ...
                hasil_kal +=' '
            if (jumlah_kata==len(kalimat_to_list(hasil_kal))):
                break
        if flag == 0:
            continue
...
def permutasi_kata(lista, data, subjek_to_objek):
    i = 1
...
#                     break
                ...
                if (data[z]=='Konj'):
                    Konjungsi.append(y)
                    break
                if ((z in data) and (data[z]=='O')) or ((z in subjek_to_objek) and (subjek_to_objek[z]=='O')):
                    Objek.append(y)
                    break
    if len(Subjek) == 0:
        Subjek.append(SS)
...
def fungsi_bedain_jenis_kal(kalimat, data, subjek_to_objek):
    Subjek, Predikat, Keterangan, Konjungsi, Objek = [], [], [], [], []
    for x in kalimat:
        for z in x:
...
def fungsi_hasil_awal (hasil_subjek, hasil_predikat, hasil_keterangan, hasil_konjungsi, 
                       hasil_objek, kalimat_ori, cocok, jumlah_kata):
    jenis = {}
    ...
    hasil_kalimat = susun_kalimat(total, kalimat2, jenis, cocok, jumlah_kata)
...
data = {'kamu':'S', 'aku':'S', 'ernia':'S', 'adik':'S', 'rifa':'S', 'lukisan':'S', 'mutia':'S', 'irfan':'S', 'ibu':'S',
        'suara':'S',
        'suka':'P', 'minum':'P', 'makan':'P', 'sangat':'P', 'sedang':'P', 'ingin':'P', 'pergi':'P', 'jatuh':'P', 'bagus':'P',
        'cinta':'P', 'mengajari':'P', 'pelan':'P',
        'yang':'P', 'itu':'P',
        'ingin pergi':'P', 'suka makan':'P', 'sedang makan':'P', 'suka minum':'P', 'sedang minum':'P', 'sangat suka':'P',
        'yang pelan':'P',
        'sangat cinta':'P',
        'sangat suka makan':'P', 'sangat suka minum':'P', 'sedang suka makan':'P', 'yang jatuh itu bagus':'P',
        'donat':'O', 'susu':'O', 'coklat':'O',
        'di':'Ket', 'rumah':'Ket', 'sekolah':'Ket', 'ke':'Ket', 'pasar':'Ket',
        'di rumah':'Ket', 'di sekolah':'Ket', 'ke pasar':'Ket',
        'ketika':'Konj', 'sedangkan':'Konj', 'tetapi':'Konj', 'dengan':'Konj'}
subjek_to_objek = {'mutia':'O', 'irfan':'O', 'kamu':'O', 'aku':'O', 'adik':'O'}
cocok = {'sedang makan':'Makan', 'donat':'Makan', 'suka makan':'Makan', 'coklat':'Makan', 'sangat suka makan':'Makan',
        'minum':'Minum', 'susu':'Minum', 'sangat suka minum':'Minum', 'sedang minum':'Minum', 'suka minum':'Minum',
        'ibu':'Person', 'mengajari':'Person', 'adik':'Person', 'aku':'Person',
         'suara':'Benda',
        'pelan':'Benda_volume', 'yang pelan':'Benda_volume'}
...
#minimum
# contoh = 'yang mengajari adik dengan suara pelan ibu'

##consent
#jadi
# contoh = 'di sekolah susu adik minum ernia sedang makan donat ketika di rumah' ##full gak ada subjek to objek
contoh = 'sangat aku mutia suka mutia tetapi cinta irfan' ## full mengandung subjek to objek

#engga
# contoh = 'ingin pergi aku ke pasar' ## subjek dan subjek to objek sama
...
# contoh = 'itu lukisan jatuh yang bagus'

kalimat2 = ['S', 'P', 'O', 'K', 'Konj', 'S', 'P', 'O', 'K']

count, jenis = pemisah_kalimat(data, contoh, j_kata, subjek_to_objek)
jumlah_kata = len(kalimat_to_list(contoh))
...
for x in permutasi_awal:
    ...
    hasil_permu_kata = permutasi_kata(semua_list, data, subjek_to_objek)
    hasil_semua_total = []
    for a in hasil_permu_kata:
        Subjekn, Predikatn, Keterangann, Konjungsin, Objekn = fungsi_bedain_jenis_kal (a, data, subjek_to_objek)
        tampungan = fungsi_hasil_awal(Subjekn, Predikatn, Keterangann, Konjungsin, Objekn, contoh, cocok, jumlah_kata)
